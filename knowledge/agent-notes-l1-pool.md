...
- **CascadeIDE / Avalonia+Dock:** при задачах UI/XAML/Dock в desktop IDE — `route_context('Avalonia'|'Dock'|'CascadeIDE UI')` или цепочка `worlds/software-dotnet-avalonia/status-avalonia-cascade-ide-ui-v1` → `worlds/software-dotnet-avalonia/playbook-avalonia-dock-ui-v1` → `worlds/software-dotnet-avalonia/kb-avalonia-ui-dock-fundamentals-v1` (см. `index-knowledge-router-v1.md`, `router-avalonia-ui`).
