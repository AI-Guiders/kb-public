...
- **`work/projects/`** — дерево по scope (`door-to-singularity`, `portal`, `harvester`, `imc`): соглашение об именах и карточки с длинным контуром вне hot-context; вход: `work/projects/README.md`. Внутри `work/projects/door-to-singularity/` — **по одному каталогу на продукт** (`cascade-ide`, `income-cascade`, `roslyn-mcp`, …), хаб workspace — `door-to-singularity/door-to-singularity/README.md`; **реестр решений** — `work/projects/door-to-singularity/solutions-registry.md`; **backlog по продуктам** — в `cascade-ide/README.md` и `roslyn-mcp/README.md` (секции Backlog); **идеи уровня workspace** — `workspace-ideas-*` в `door-to-singularity/`.
