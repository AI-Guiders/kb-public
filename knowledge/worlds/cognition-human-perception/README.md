...
- **HCI** (`playbook-hci-core-v1.md`, `kb-hci-usability-and-dialog-rules-v1.md`) — прикладные контракты взаимодействия; human-perception даёт **почему** часть правил работает под нагрузкой.
- **Домен Psychology** (клинические модели, терапия) — **не** заменяется этим миром; при психическом кризисе — `kb-therapy-and-support-boundaries-v1.md` и границы из роутера.
- **Aviation Human Factors** + `kb-aviation-pfd-mfd-efis-eicas-fundamentals-v1.md` — метафора кокпита в IDE (Cascade) **пересекается** по вниманию и SA; детали размещения — в ADR продукта, не здесь.
- **DE/DX / UI/UX** — числовые бюджеты и продуктовые решения в репозитории; KB даёт **слой восприятия** перед спором «куда кнопку».

## Версия

v1.4 · 2026-04-20 — имена файлов `*-evidence-v1`
