...
Развёрнутые примеры оформления и привычки мультирепо: **`playbook-multi-project-context-v1.md`**; полный текст протокола с **инсталляционными** таблицами — у автора канона в **`agent-notes.md`** (**`project-switch-protocol-v1`**; в kb-public фрагмент может не попасть из-за **`public-cut`** — критичное дублируй у себя в hot).
